<?php
/**
 * UNQVerify Age Verification — admin layer.
 *
 * Named callback functions that WooCommerce/WordPress hook systems reference
 * by string must live at global scope, so they are defined here before the
 * UNQ_Admin class. The class itself only registers add_action/add_filter.
 *
...
/**
 * Render the Aldersverificering settings tab content.
 *
...
function unq_agev_render_settings_page() {
    ...
    <div class="unq-help-bar">
        <span><?php esc_html_e( 'Need help?', 'unq-age-verification' ); ?></span>
        <a href="https://www.aldersverificering.dk/docs/woocommerce" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Read the docs', 'unq-age-verification' ); ?> &rarr;</a>
        <span class="unq-help-sep">&middot;</span>
        <a href="https://www.aldersverificering.dk/kontakt" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Contact support', 'unq-age-verification' ); ?> &rarr;</a>
    </div>
    ...
    <div id="unq-save-bar" role="status" aria-live="polite">
        <p><?php esc_html_e( 'You have unsaved changes', 'unq-age-verification' ); ?></p>
        <button type="button" id="unq-save-bar-btn"><?php esc_html_e( 'Save changes', 'unq-age-verification' ); ?></button>
    </div>
    ...
    (function () {
        ...
        // Block form submit on invalid keys.
        ...
        var saveBar    = document.getElementById('unq-save-bar');
        var saveBarBtn = document.getElementById('unq-save-bar-btn');
        var formDirty  = false;
        ...
        if ( saveBarBtn && form ) {
            saveBarBtn.addEventListener('click', function () {
                // Click WC's native save button so our validation listener also fires.
                var wcBtn = form.querySelector('[name="save"]');
                if ( wcBtn ) { wcBtn.click(); } else { form.submit(); }
            });
        }
    })();
    </script>
...
// ---------------------------------------------------------------------------
// Category term meta save callback.
// ---------------------------------------------------------------------------

/**
 * Save unq_agev_category_required term meta when a product_cat term is created
 * or updated via the per-term edit screen.
 *
 * @param int $term_id
 */
...
class UNQ_Admin {
    ...
    public static function register() {
        ...
        add_action( 'admin_init', function () {
            ...
            register_setting( $group, 'unq_agev_enabled', array(
                'type'              => 'string',
                'sanitize_callback' => function ( $v ) { return 'yes' === $v ? 'yes' : 'no'; },
                'default'           => 'yes',
            ) );
            register_setting( $group, 'unq_agev_public_key', array(
                'type'              => 'string',
                'sanitize_callback' => 'sanitize_text_field',
                'default'           => '',
            ) );
            register_setting( $group, 'unq_agev_test_public_key', array(
                'type'              => 'string',
                'sanitize_callback' => 'sanitize_text_field',
                'default'           => '',
            ) );
            register_setting( $group, 'unq_agev_use_production', array(
                'type'              => 'string',
                'sanitize_callback' => function ( $v ) { return 'yes' === $v ? 'yes' : 'no'; },
                'default'           => 'no',
            ) );
            register_setting( $group, 'unq_agev_required_age', array(
                'type'              => 'integer',
                'sanitize_callback' => function ( $v ) { return max( 1, min( 120, (int) $v ) ); },
                'default'           => 18,
            ) );
            register_setting( $group, 'unq_agev_verification_mode', array(
                'type'              => 'string',
                'sanitize_callback' => function ( $v ) {
                    return in_array( $v, array( 'popup', 'redirect' ), true ) ? $v : 'popup';
                },
                'default'           => 'popup',
            ) );
            register_setting( $group, 'unq_agev_locale', array(
                'type'              => 'string',
                'sanitize_callback' => function ( $v ) {
                    return in_array( $v, array( 'auto', 'en', 'da' ), true ) ? $v : 'auto';
                },
                'default'           => 'auto',
            ) );
            register_setting( $group, 'unq_agev_targeting', array(
                'type'              => 'string',
                'sanitize_callback' => function ( $v ) {
                    return in_array( $v, array( 'all', 'selected_only' ), true ) ? $v : 'all';
                },
                'default'           => 'all',
            ) );
        } );
        ...
        add_filter( 'woocommerce_settings_tabs_array', function ( $tabs ) {
            $tabs['unq_agev'] = __( 'Aldersverificering', 'unq-age-verification' );
            return $tabs;
        }, 50 );
        ...
        add_action( 'admin_enqueue_scripts', function ( $hook ) {
            ...
            $on_settings     = 'woocommerce_page_wc-settings' === $hook
                               && isset( $_GET['tab'] ) && 'unq_agev' === sanitize_key( $_GET['tab'] );
            $on_product_list = ( 'edit.php' === $hook && isset( $_GET['post_type'] ) && 'product' === sanitize_key( $_GET['post_type'] ) );
            $on_product_edit = ( 'post.php' === $hook || 'post-new.php' === $hook )
                               && ( ( isset( $_GET['post_type'] ) && 'product' === sanitize_key( $_GET['post_type'] ) )
                                    || ( isset( $_GET['post'] ) && 'product' === get_post_type( (int) $_GET['post'] ) ) );
            $on_cat_screen   = ( 'edit-tags.php' === $hook || 'term.php' === $hook )
                               && isset( $_GET['taxonomy'] ) && 'product_cat' === sanitize_key( $_GET['taxonomy'] );

            if ( ! $on_settings && ! $on_product_list && ! $on_product_edit && ! $on_cat_screen ) {
                return;
            }

            // Use file mtime when WP_DEBUG is on so CSS changes are always picked up
            // in development without needing a version bump.
            $css_ver = ( defined( 'WP_DEBUG' ) && WP_DEBUG )
                ? filemtime( UNQ_AGEV_PATH . 'assets/admin.css' )
                : UNQ_AGEV_VERSION;
        ...
        // ── Products list: term-meta cache pre-warm (eliminates N+1) ─────
        add_filter( 'the_posts', function ( $posts, $query ) {
            if ( ! is_admin() || ! $query->is_main_query() ) {
                return $posts;
            }
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            if ( empty( $_GET['post_type'] ) || 'product' !== sanitize_key( $_GET['post_type'] ) ) {
                return $posts;
            }
            if ( empty( $posts ) ) {
                return $posts;
        ...
        // ── Product categories list column ────────────────────────────────
        ...
        add_filter( 'plugin_row_meta', function ( $links, $file ) {
            if ( UNQ_AGEV_BASENAME !== $file ) {
                return $links;
            }
            $links[] = '<a href="https://www.aldersverificering.dk/docs/woocommerce" target="_blank" rel="noopener noreferrer">'
                . esc_html__( 'Docs', 'unq-age-verification' )
                . '</a>';
            ...
            printf(
                ...
                wp_kses(
                    sprintf(
                        ...
                        __( '<strong>Aldersverificering:</strong> Ingen API-nøgle er konfigureret. <a href="%s">Tilføj din nøgle under WooCommerce &rarr; Indstillinger &rarr; Aldersverificering</a>.', 'unq-age-verification' ),
