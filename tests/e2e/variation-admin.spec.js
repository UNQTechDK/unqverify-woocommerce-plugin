...
test('variation age-verification controls render and persist in wp-admin', async ({ page }) => {
  await logIn(page);
  ...
  await expect(parentNotice).toContainText('2 variations have direct age-verification rules');

  const variationsTab = page.locator('.product_data_tabs a[href="#variable_product_options"]');
  await variationsTab.scrollIntoViewIfNeeded();
  await variationsTab.click();
  await expect(page.locator('input.variable_post_id[value="23"]')).toBeAttached();

  const restrictedRow = await variationRow(page, 23);
  await restrictedRow.locator('.edit_variation').click();
  const restrictedToggle = restrictedRow.locator('.unq-agev-variation-required');
  ...
  const unrestrictedToggle = unrestrictedRow.locator('.unq-agev-variation-required');
  ...
  const saveResponse = page.waitForResponse(
    (response) => response.request().method() === 'POST' && response.url().includes('/wp-admin/post.php'),
  );
  await page.locator('#publish').click();
  await saveResponse;
  await expect(page.locator('div.notice-success#message')).toContainText(/Product (updated|published)/);

  const persistedVariationsTab = page.locator('.product_data_tabs a[href="#variable_product_options"]');
  await persistedVariationsTab.scrollIntoViewIfNeeded();
  await persistedVariationsTab.click();
  await expect(page.locator('input.variable_post_id[value="22"]')).toBeAttached();
  const persistedRow = await variationRow(page, 22);
  await persistedRow.locator('.edit_variation').click();
  await expect(persistedRow.locator('.unq-agev-variation-required')).toBeChecked();
});