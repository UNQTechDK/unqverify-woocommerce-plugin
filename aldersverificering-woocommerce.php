...
// ---------------------------------------------------------------------------
// Settings API registration — register_setting() per WP plugin best practices.
// Sanitize callbacks mirror the validation in the custom WC save handler.
// The custom WC tab UI is preserved; this registration satisfies WP.org review.
// ---------------------------------------------------------------------------
...
add_action( 'woocommerce_update_options_unq_agev', function () {
    ...
    // Explicit capability check — belt-and-suspenders over WooCommerce's own nonce.
...
function unq_agev_save_category_meta( $term_id ) {
    // WordPress verifies its own term-edit nonce before firing created_product_cat /
    // edited_product_cat. Add an explicit capability check per WP security guidelines.
...
// ---------------------------------------------------------------------------
// Products list — prime term-meta cache to eliminate N+1 per-product queries.
// Each call to get_term_meta() is a DB round-trip unless we batch-warm the
// cache with update_termmeta_cache() before the column callback fires.
// ---------------------------------------------------------------------------
add_filter( 'the_posts', function ( $posts, $query ) {
    ...
    // Collect all product_cat term IDs for this page of products.
    $term_ids = array();
...
// ---------------------------------------------------------------------------
// Invalidate per-request static caches when cart contents change mid-request.
// Plugins such as free-gift or auto-coupon plugins can add/remove cart items
// after our gating check has already run. These hooks ensure the result is
// always fresh after any cart mutation within the same request.
// ---------------------------------------------------------------------------
